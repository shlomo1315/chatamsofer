 ...
 *
 * 🔴 רווח בלבד, בלי פסיק: "רחוב הרב קוק 4 ירושלים".
 *
 * ⚠️ פסיק שבא מיד אחרי מספר בית נבלע ב-LTR_TOKEN של lib/pdfBidi ומתהפך
 * יחד עם המספר — "קוק 4, ירושלים" הוצג "קוק ,4 ירושלים". גם רווח דק
 * (U+2009) נבלע. לכן ויתרנו על הפסיק: רווח קורא נכון ואינו יכול להתהפך.
 */
function fullAddress(b: { address?: string | null; city?: string | null }): string {
  return [b.address, b.city].map(v => String(v ?? '').trim()).filter(Boolean).join(' ')
}
