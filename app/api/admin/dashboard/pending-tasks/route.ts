  ...
  try {
    ...
    const [beneficiaries, loans, maternity, widows, financial] = await Promise.all([
      supabase.from('beneficiaries')
        ...
        .limit(30),
      supabase.from('loans')
        .select('id, created_at, beneficiary:beneficiary_id(full_name)')
        .eq('status', 'pending')
        .order('created_at', { ascending: false })
        .limit(30),
      ...
      supabase.from('widow_requests')
        .select('id, created_at, request_type, beneficiary:beneficiary_id(full_name)')
        .eq('status', 'pending')
        .order('created_at', { ascending: false })
        .limit(30),
    ...
    const tasks = [
      ...
      ...(loans.data ?? []).map(l => ({
        ...
        name: (l.beneficiary as Ben)?.full_name ?? 'לא ידוע',
        detail: 'בקשת הלוואה',
      ...
      ...(maternity.data ?? []).map(m => ({
        ...
        name: (m.beneficiary as Ben)?.full_name ?? 'לא ידוע',
        detail: 'בקשת יולדת',
      ...
      ...(widows.data ?? []).map(w => ({
        ...
        name: (w.beneficiary as Ben)?.full_name ?? 'לא ידוע',
      ...
      ...(financial.data ?? []).map(f => ({
        ...
        name: (f.beneficiary as Ben)?.full_name ?? 'לא ידוע',
