...
export async function PATCH(request: NextRequest) {
  ...
  // 🔴 אישור האבות אינו אוטומטי יותר — רק בבקשה מפורשת.
  //
  // אישור צומת אחד סימן "מאושר" את *כל* שרשרת האבות הממתינה מעליו, בשקט. מי
  // שאישר אדם בדור 9 אישר איתו ארבעה או חמישה אבות שאיש לא בדק — וזה מסוכן
  // במיוחד בזמן ניקוי כפילויות, כי בדיוק האבות הכפולים שנבדקים נצבעו ירוק.
  //
  // ⚠️ הצורך אמיתי ולכן האפשרות נשארה: בורר סדר הדורות ברישום מנווט דרך צמתים
  // מאושרים בלבד, ולכן בן מאושר שאביו "ממתין" פשוט לא יופיע לנרשמים הבאים.
  // המסך מציג את רשימת האבות ושואל; כאן רק מבצעים את מה שנענה.
  if (updates.status === 'verified' && body.verifyAncestors === true) {
