...
// ⚠️ נקודה-פסיק נכללת: היא מפריד ההשהיה של *השיחה היוצאת* (lib/yemotCall),
// ובשלוחה אין לה מקום — כאן ההשהיה נעשית בטוקנים נפרדים. בלי הסרתה היא הייתה
// הופכת לטוקן משל עצמה ומוקראת בקול.
const TTS_INVALID = /[.,;\-"'&|=]/g
...
// ⚠️ 6 טוקנים בין ספרה לספרה *החמירו* את המצב: רשימת ההודעות (id_list_message)
// נעשתה ארוכה מדי, וימות הקריאה אותה במהירות במקום להשהות. לכן ההשהיה מושגת
// בשתי דרכים יחד ובמידה:
//   1. שני טוקני-השהיה בין ספרה לספרה (הפסקה בין הודעה להודעה).
//   2. פסיקים *בתוך* טוקן הספרה עצמה — אותה שיטה שעובדת בשלוחת היולדות
//      (slowTokenOf), שם ההקראה נשמעת איטית וברורה.
// כך הרשימה נשארת קצרה וההקראה ספרה-אחרי-ספרה.
// ⚠️ טוקן השהיה נוסף *לפני כל ספרה*, ולא רק בין שתי ספרות עוקבות: מאז
// שההקראה כוללת מילת מקום לפני כל ספרה ("ספרה שנייה"), שתי ספרות אינן עוקבות
// עוד — והתנאי הישן הפסיק להוסיף השהיות בכלל. הכמות הופחתה ל-1 כדי שאורך
// רשימת ההודעות יישאר כשהיה: רשימה ארוכה מדי גורמת לימות להקריא מהר יותר.
const PAUSE_TOKENS_BETWEEN_DIGITS = 1
/** פסיקים בתוך טוקן הספרה — מאריכים את ההשהיה בלי להאריך את רשימת ההודעות */
const DIGIT_TAIL = ' , , ,'
function slowTokens(text: string): string {
  ...
  for (const w of words) {
    if (DIGIT_SET.has(w)) {
      ...
      tokens.push(`t-${w}${DIGIT_TAIL}`)   // הספרה עצמה + השהיה פנימית
      lastWasDigit = true
    } else {
      buf.push(w)
      lastWasDigit = false
 ...
 * הנשמר ב-app_settings תחת verify:phone:<טלפון מנורמל>.
 * מחזיר את הקוד ומוחק אותו מהרשומה (הקראה חד-פעמית), או null אם אין/פג תוקף.
 * ⚠️ ה-hash נשאר — בלעדיו האימות בטופס נשבר.
 */
...
async function handle(req: NextRequest) {
  ...
  if (!row || !row.portal_phone_code_plain) {
    ...
    // שמסלול הכניסה לא מצא דבר — ולכן הכניסה אינה מושפעת כלל.
    const pending = await readPendingVerifyCode(admin, caller)
    if (pending) {
      console.log(`[yemot-otp] reading signup code callId=${callId}`)
      return hangupSlow(spokenCode(pending), callId)
    }
