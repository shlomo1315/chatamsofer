import { verifyPublicToken } from '@/lib/publicToken'
import { createClient } from '@supabase/supabase-js'
import FixNameForm from './FixNameForm'

export const dynamic = 'force-dynamic'

// עמוד ציבורי — השלמת / תיקון שם התינוק ע"י היולדת, דרך קישור אישי שנשלח למייל.
export default async function Page({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params
  const aidId = verifyPublicToken(token, 'n')

  if (!aidId) {
    return (
      <main dir="rtl" className="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-rose-50">
            <svg className="h-7 w-7 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m0 3.75h.007M10.34 3.94l-7.6 13.17A1.5 1.5 0 004.04 19.5h15.92a1.5 1.5 0 001.3-2.39l-7.6-13.17a1.5 1.5 0 00-2.6 0z" />
            </svg>
          </div>
          <h1 className="text-xl font-bold" style={{ color: '#1B3256' }}>הקישור אינו תקין</h1>
          <p className="mt-3 text-sm leading-relaxed text-slate-600">
            ייתכן שהקישור פג תוקף (הקישור תקף 7 ימים) או הועתק באופן חלקי.
            <br />
            נשמח אם תנסו שוב מהקישור המקורי שקיבלתם, או לפנות למשרד.
          </p>
        </div>
      </main>
    )
  }

  // שליפת שם התינוק הנוכחי (אם קיים) — למילוי מראש / חיווי
  let currentName = ''
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY
  if (url && key) {
    const admin = createClient(url, key, { auth: { autoRefreshToken: false, persistSession: false } })
    const { data } = await admin.from('maternity_aids').select('baby_name, baby_gender').eq('id', aidId).maybeSingle()
    if (data?.baby_name) currentName = String(data.baby_name)
  }

  return <FixNameForm token={token} currentName={currentName} />
}
