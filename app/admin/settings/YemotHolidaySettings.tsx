...
export default function YemotHolidaySettings() {
  ...
  const save = async () => {
    ...
    try {
      ...
      //
      // ⚠️ כל הודעה שהטקסט שלה השתנה — לא רק כאלה שכבר יש להן קול.
      // המנהל ביקש במפורש שכל ההקלטות יעברו דרך הקול הטבעי, ולכן
      // שמירה מייצרת גם להודעה שהייתה עד כה בהקראה ממוחשבת.
      //
      // ⚠️ רק מה שהשתנה: כל שמירה שהייתה מייצרת מחדש את כל 26 ההודעות
      // הייתה מבזבזת מכסת ElevenLabs על טקסט שלא נגעו בו.
      //
