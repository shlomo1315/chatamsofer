...
//
// 🔴 ה-TTL לבדו אינו מספיק — הוא פוקע רק כשמישהו קורא ל-loadEmailTexts.
// ההערה ב-textFor הבטיחה שהקריאה מתבצעת "ב-deliverMail, לפני כל שליחה",
// אבל sendMail.ts מעולם לא ייבא את המודול הזה. בפועל הטקסטים נטענו רק
// בעליית השרת, ועריכה במסך ההגדרות לא השפיעה על מיילים שנשלחו מתהליך
// אחר עד לפריסה הבאה. ראו ensureEmailTexts למטה.
const TTL_MS = 10_000
let fetchedAt = 0
...
/**
 * 🔴 יש לקרוא לזה לפני *בניית* כל תבנית מייל.
 *
 * ⚠️ ולא לפני השליחה: deliverMail מקבל HTML מוכן, ובשלב הזה הטקסטים כבר
 * הוטמעו בו. טעינה שם הייתה מרעננת את המטמון בדיוק אחרי שהשתמשו בו.
 *
 * ⚠️ אינו זורק לעולם ואינו חוסם: אם הטעינה נכשלת, textFor מחזיר את
 * ברירות המחדל שבקוד והמייל יוצא עם הנוסח המקורי. עדיף מייל בנוסח ישן
 * מאשר מייל שלא נשלח.
 */
export async function ensureEmailTexts(): Promise<void> {
  try { await loadEmailTexts() } catch { /* ברירות המחדל שבקוד */ }
}
