...
export function audioToken(a?: IvrAudio | null): string {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // 🔴 הקובץ *וגם* הטקסט — ולא הקובץ בלבד.
  //
  // ⚠️ ימות מדלגת בשקט על אסימון f- שהקובץ שלו אינו קיים, וממשיכה
  // לאסימון הבא. כשהוחזר רק f-, שלוחה שהקובץ שלה נמחק או שונה שם
  // השמיעה *שקט מוחלט* — המתקשר לא ידע מה להקיש, ההקשה נדחתה, והשיחה
  // חזרה על עצמה עד שהתנתקה. אין שגיאה בשום צד.
  //
  // כך בדיוק מת התפריט אחרי שהוספנו חותמת זמן לשמות הקבצים: ההפניות
  // הישנות (ivr_root_prompt) הצביעו על שמות שכבר אינם קיימים.
  //
  // ⚠️ 🔴 אין כאן גיבוי משולב: ימות משמיעה *את שניהם* כשהקובץ קיים,
  // כלומר כל מתקשר שומע את ההודעה פעמיים. הפתרון הנכון הוא לא להשאיר
  // הפניה לקובץ שאינו קיים — ראו verifyAudio ב-yemot-ivr.
  // ─────────────────────────────────────────────────────────────────────────
...
function enterNode(
  ...
  switch (node.type) {
    case 'menu':
      // ─────────────────────────────────────────────────────────────────
      // 🔴 תפריט בלי מקשים אינו תפריט.
      //
      // ⚠️ readMenu בונה את רשימת המקשים המותרים מ-node.keys. כשהיא
      // ריקה נשלחת לימות פקודת read בלי אף ספרה מותרת, וימות עונה
      // "שגיאה, לא הוקשה בחירה" ומנתקת — גם כשהמתקשר הקיש כראוי,
      // כי שום הקשה אינה חוקית.
      //
      // ⚠️ משמיעים את ההודעה ומנתקים בשקט במקום להשמיע שגיאה: המתקשר
      // לא עשה דבר רע, והתקלה היא בהגדרה. הבנאי מסמן את זה כשגיאה
      // ("תפריט בלי מקשים") — ראו validateIvr.
      // ─────────────────────────────────────────────────────────────────
      if (!(node.keys ?? []).length) {
        return {
          commands: [
