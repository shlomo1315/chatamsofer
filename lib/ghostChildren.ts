...
/**
 * 🔴 האם הוכח שהאדם שמאחורי הצומת הזה כבר קיים בעץ — כלומר הצומת הוא עותק
 * מיותר, ולא מקומו היחיד של אדם.
 *
 * זו ההבחנה שקובעת מה מותר להסיר, והיא מפרידה בין שני דברים שנראים זהים:
 *
 *   • צומת עם תאום, או שכרטסתו יושבת על צומת אחר — האדם *נמצא* במקום אחר
 *     בעץ. הצומת הזה כפול, והסרתו אינה מוחקת אף אחד.
 *
 *   • צומת בלי כרטסת ובלי תאום — אין שום ראיה שהאדם קיים במקום אחר. הצומת
 *     הזה נוצר בכוונה כדי שהילד ימצא את עצמו כשיבוא להירשם, והסרתו תגרום
 *     בדיוק לכפילות שהמנגנון נועד למנוע. ⚠️ אלה הרוב, ואין לגעת בהם.
 *
 * ההבחנה כאן ולא בממשק, כדי שאף קורא לא יוכל להסיר צומת שאין עליו ראיה.
 */
export function isProvenDuplicate(r: GhostRow): boolean {
  return r.group === 'card_elsewhere' || r.twinNodeId != null
}
