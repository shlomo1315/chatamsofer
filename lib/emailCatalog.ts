...
export const EMAIL_CATALOG: EmailSpec[] = [
  ...
  {
    id: 'holiday_call_notice',
    group: 'registration',
    title: 'חלוקות חגים — חובה להירשם בטלפון',
    trigger: 'אחרי רישום המשפחה במערכת, כשהרישום לחלוקת חגים פתוח',
    recipient: 'המשפחה',
    department: 'igud',
    wired: true,
    fields: [
      { key: 'subject', label: 'שורת הנושא', default: 'חשוב! רישום לחלוקת החגים — היכל החתם סופר' },
      { key: 'preheader', label: 'שורת התצוגה המקדימה', default: 'ללא רישום טלפוני לא תהיו זכאים לחלוקת החגים.' },
      { key: 'title', label: 'כותרת ראשית', default: 'רישום לחלוקת החגים' },
      { key: 'subtitle', label: 'כותרת משנה', default: 'היכל החתם סופר · חלוקות חגים' },
      { key: 'intro', label: 'פסקת פתיחה', multiline: true, default: 'הרשמתכם נקלטה בהצלחה.' },
      { key: 'phone_title', label: 'כותרת בלוק הטלפון', multiline: true, default: 'שימו לב: לצורך הרישום לחלוקת חג תשרי הבעל״ט יש לחייג למספר הטלפון שלהלן ולפעול לפי ההנחיות.' },
      { key: 'phone', label: 'מספר הטלפון לרישום', default: '' , hint: 'המספר מוצג בהדגשה במרכז המייל' },
      { key: 'warning', label: 'אזהרה', multiline: true, default: 'ללא רישום טלפוני לא תהיו זכאים לחלוקת החגים — גם אם השלמתם את הרישום לאיגוד הצאצאים!' },
      { key: 'deadline', label: 'מועד אחרון', multiline: true, default: 'הרישום לחלוקת החגים יתקיים עד לתאריך כ״ז אב הבעל״ט. לאחר מכן לא נוכל לקבל בקשות חדשות.' },
    ],
  },
  {
    id: 'holiday_registered',
    group: 'registration',
    title: 'חלוקות חגים — אישור רישום',
    trigger: 'מיד לאחר שהמשפחה נרשמה לחלוקה (ממשק / טלפון / מייל)',
    recipient: 'המשפחה',
    department: 'igud',
    wired: true,
    fields: [
      { key: 'subject', label: 'שורת הנושא', default: 'רישומכם לחלוקת החגים נקלט — היכל החתם סופר' },
      { key: 'preheader', label: 'שורת התצוגה המקדימה', default: 'הרישום לחלוקת החגים נקלט בהצלחה.' },
      { key: 'title', label: 'כותרת ראשית', default: 'הרישום נקלט בהצלחה' },
      { key: 'subtitle', label: 'כותרת משנה', default: 'היכל החתם סופר · חלוקות חגים' },
      { key: 'intro', label: 'פסקת פתיחה', multiline: true, default: 'רישומכם לחלוקת החגים נקלט בהצלחה.' },
      { key: 'next', label: 'מה צפוי בהמשך', multiline: true, default: 'שימו לב: בעזרת השם, במהלך חודש אלול תקבלו עדכון מדויק על אופן החלוקה. אין צורך בפעולה נוספת מצדכם.' },
    ],
  },
  {
    id: 'docs_pending',
  ...
  },
  {
    id: 'book_fair_opened',
    group: 'book_fair',
    title: 'היריד נפתח — תזכורת',
    trigger: 'ברגע פתיחת היריד, לכל מי שהשאיר כתובת בדף ההמתנה',
    recipient: 'נרשמי התזכורת',
    department: 'yerid',
    wired: true,
    fields: [
      { key: 'subject', label: 'שורת הנושא', default: 'יריד הספרים נפתח — אפשר להזמין' },
      { key: 'preheader', label: 'שורת התצוגה המקדימה', default: 'המערכת פתוחה להזמנות.' },
      { key: 'title', label: 'כותרת ראשית', default: 'היריד נפתח' },
      { key: 'kicker', label: 'תווית קטנה מעל הפתיח', default: 'תזכורת' },
      { key: 'headline', label: 'כותרת הפתיח', default: 'מערכת ההזמנות פתוחה' },
      { key: 'intro', label: 'פסקת פתיחה', default: 'ביקשתם שנזכיר — מערכת הזמנת ספרי החתם סופר נפתחה, וניתן להזמין כעת.', multiline: true },
      { key: 'cta', label: 'כיתוב הכפתור', default: 'למעבר לחנות' },
      { key: 'footnote', label: 'הערה בתחתית', default: 'קיבלתם מייל זה משום שביקשתם תזכורת בדף ההמתנה. זו הודעה חד-פעמית.', multiline: true },
    ],
  },
  {
    id: 'benefits_link',
    ...
    fields: [
      ...
      { key: 'btn_aid', label: 'כפתור — סיוע רפואי', default: 'להגשת בקשת סיוע רפואי — לחצו כאן' },
      { key: 'btn_widow', label: 'כפתור — אלמנות ויתומים', default: 'להגשת בקשה לאלמנות ויתומים — לחצו כאן' },
      { key: 'btn_holiday', label: 'כפתור — רישום לחלוקת חגים', default: 'להרשמה לחלוקת החגים — לחצו כאן' },
      { key: 'btn_details', label: 'כפתור — עדכון פרטים אישיים', default: 'לעדכון הפרטים האישיים שלכם (טלפון, מייל, כתובת ושמות הילדים) — לחצו כאן' },
      { key: 'draft_title', label: 'כותרת בלוק ההגשה במייל', default: 'להגשה גם דרך האימייל:' },
      { key: 'draft_note', label: 'הסבר בלוק ההגשה במייל', default: 'ניתן להגיש טופס בקשה גם דרך האימייל. עם זאת, היות והמערכת עדיין בפיתוח ייתכנו בהגשה דרך האימייל שיבושים רבים. אם בחרתם באפשרות זו הקישו על הלחצנים דלהלן ותקבלו את טפסי הבקשה באימייל:', multiline: true },
    ],
  },
  ...
  // ── יולדות ────────────────────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      { key: 'card_title', label: 'כותרת כרטיס המזון', default: 'כרטיס מזון על סך 600 ₪' },
      { key: 'card_text', label: 'טקסט כרטיס המזון', default: 'מצורף שובר לאיסוף כרטיס המזון. יש להדפיס את השובר ולהביאו לאחד מהמוקדים לקבלת הכרטיס.', multiline: true },
