...
// ─────────────────────────────────────────────────────────────────────────────
// 🔴 הרגרסיה: "שגיאה, לא הוקשה בחירה" מיד בתחילת השיחה.
//
// readMenu בונה את רשימת הספרות המותרות מ-node.keys. בתפריט בלי מקשים
// הרשימה ריקה, ולכן נשלחת לימות פקודת read שאין בה אף הקשה חוקית —
// וימות עונה "לא הוקשה בחירה" ומנתקת, גם כשהמתקשר הקיש כראוי.
//
// ⚠️ זה קורה דווקא בשלוחה שנבנתה זה עתה, לפני שהוגדרו לה מקשים — כלומר
// בדיוק ברגע שבו המנהל בודק אם הבנייה עבדה.
// ─────────────────────────────────────────────────────────────────────────────
describe('🔴 תפריט בלי מקשים', () => {
  const empty = cfg([node({ id: 'root', type: 'menu', prompt: { text: 'ברוכים הבאים' }, keys: [] })])

  it('אינו שולח פקודת read בלי ספרות מותרות', () => {
    const out = reply(empty)
    // read עם רשימת ספרות ריקה = "לא הוקשה בחירה" בכל הקשה.
    expect(out).not.toMatch(/^read=/)
  })

  it('משמיע את ההודעה שהוגדרה', () => {
    expect(reply(empty)).toContain('id_list_message=')
  })

  it('מסיים את השיחה במקום להיתקע', () => {
    expect(reply(empty)).toContain('go_to_folder=hangup')
  })

  it('תפריט *עם* מקשים ממשיך לקלוט הקשה כרגיל', () => {
    const ok = cfg([
      node({ id: 'root', type: 'menu', prompt: { text: 'תפריט' }, keys: [{ digit: '1', target: 'a' }] }),
      node({ id: 'a', prompt: { text: 'שלוחה' } }),
    ])
    expect(reply(ok)).toMatch(/^read=/)
  })
})
