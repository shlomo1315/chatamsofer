...
// ─────────────────────────────────────────────────────────────────────────────
// 🔴 הדרכון נשמר בנדרים בשדה Tsad3Id, והחיפוש חייב להסתכל בו.
//
// ⚠️ הבאג שהבדיקות האלה שומרות עליו (17.09): הקוד שלח את הדרכון בשדה
// Zeout3. נדרים מחזירה Result=OK ומתעלמת ממנו — הכרטסת נוצרה בלי שום
// מזהה. מכיוון ש-findClientByZeout מחפשת לפי מזהה, המשפחה לא נמצאה
// בפנייה הבאה, נראתה כחדשה, והוקמה שוב: כרטסת אחת קיבלה את הכרטיס
// והשנייה את הכסף. 4 משפחות נדחו בקופה ב"אין תלושים זמינים למימוש
// בכרטיס זה" (בראנדער, קליין נחום יואל, שווארץ, שטרן מאיר).
//
// 🔴 שני חצאים לתיקון אחד — תיקון הכתיבה לבדו אינו מספיק:
//   · saveClientCard שולחת Tsad3Id (אומת מול נדרים: Zeout3 אינו נשמר)
//   · findClientByZeout מזהה גם שם שדה שאין בו "zeout"
// ─────────────────────────────────────────────────────────────────────────────
describe('איתור משפחה לפי דרכון בשדה צד ג', () => {
  // הסינון בפועל מתוך findClientByZeout — מקור אמת יחיד לבדיקה.
  const FIELD_RE = /zeout|tz\b|teudat|tsad3/i

  it('🔴 Tsad3Id נכלל בסינון — בלעדיו דרכון אינו נמצא לעולם', () => {
    expect(FIELD_RE.test('Tsad3Id')).toBe(true)
  })

  it('שדות הת"ז הרגילים ממשיכים להיתפס', () => {
    for (const k of ['Zeout', 'Zeout2', 'TZ', 'TeudatZehut']) {
      expect(FIELD_RE.test(k)).toBe(true)
    }
  })

  it('שדות שאינם מזהים אינם נתפסים', () => {
    for (const k of ['FamilyName', 'Phone1', 'Address', 'Email', 'Amount']) {
      expect(FIELD_RE.test(k)).toBe(false)
    }
  })

  it('🔴 נרמול דרכון משמר אותיות — בשני צדי ההשוואה', () => {
    // ⚠️ המספר בנדרים עשוי לכלול מקף או רווח ואצלנו לא (GD-3551005).
    // נרמול שמסיר אותיות היה מרוקן דרכון לגמרי ומשווה מחרוזות ריקות.
    expect(normalizeZeout('GD-3551005')).toBe('GD3551005')
    expect(normalizeZeout('gd 3551005')).toBe('GD3551005')
    expect(normalizeZeout('GB4843288')).toBe('GB4843288')
    // ת"ז מרופדת ל-9 ספרות, כמו קודם
    expect(normalizeZeout('3400')).toBe('000003400')
  })
})
