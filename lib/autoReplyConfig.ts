...
export function requestMailtoUrl(subject: string, mailbox?: string): string {
  ...
  const body =
    ...
    `‏──────────────────────────\n` +
    `בשורת הנושא למעלה, יש למחוק את הסוגריים ואת הטקסט שבתוכם,\n` +
    `ולהקליד במקומם מספר תעודת זהות תקין בלבד — 9 ספרות, כולל ספרת ביקורת.\n\n` +
    `אין להוסיף בשורת הנושא שום מלל אחר, ואין למחוק את "${subject}".\n` +
    `מספר שאינו תקין או שורת נושא שהשתנתה — הבקשה לא תיקלט במערכת.\n` +
    `‏──────────────────────────\n\n` +
    `פרטי הפונה:\n\n` +
    `שם מלא: \n` +
    `מספר זהות: \n` +
