...
export const HOLIDAY_MESSAGE_META: HolidayMsgMeta[] = [
  ...
  // בבית מספר שאינו קיים, והנחת היסוד שלו הייתה שהוא פספס משהו.
  {
    ...
    hint: 'נשמע במקש 3 כשהאיסוף סגור *לפני* שהחל (pickup_open=false ואיש טרם אסף).',
  },
  // ─────────────────────────────────────────────────────────────────────────
  // 🔴 "טרם נפתח" ו"כבר הסתיים" הם אותו מתג סגור — ומשפט הפוך למתקשר.
  //
  // ⚠️ עד כה נשמע "עדיין לא נפתחה" גם אחרי שהחלוקה הסתיימה: מי שאיחר
  // שמע שעליו להמתין, נשאר לחכות להודעה שלא תגיע, ולא ידע שהמועד עבר.
  // ההבחנה היא האם כבר חולק ולו כרטיס אחד (pickup_open_at על מוקד כלשהו).
  // ─────────────────────────────────────────────────────────────────────────
  {
    key: 'card_pickup_ended', label: 'החלוקה הסתיימה — כל המערכת', allowAudio: true,
    defaultText: 'החלוקה כבר הסתיימה, אין אפשרות לקבל כעת כרטיס. עמכם הסליחה',
    hint: 'נשמע כשהאיסוף נסגר בכל המערכת *אחרי* שכבר חולקו כרטיסים. בלי שם מוקד — ולכן ניתן להקלטה.',
  },
  {
    // 🔴 שם המוקד נאמר במפורש — כמו ב"טרם החל".
    //
    // ⚠️ המוקדים אינם מסיימים באותו יום. "החלוקה הסתיימה" בלי שם גורם
    // למי ששומע לחשוב שהכול נגמר בכל הארץ, בזמן שהמוקד של שכנו עדיין
    // מחלק — והוא מתקשר למשרד לברר. השם הוא כל ההבדל.
    //
    // ⚠️ דינמית ולכן allowAudio:false — שם המוקד משתנה בין מאזין למאזין.
    // 🔴 "המוקד שלכם" ולא שם המוקד — וזה כל ההבדל.
    //
    // ⚠️ נוסח עם {center} נקרא כולו ב-TTS של ימות, קול משובש בעברית.
    // "שלכם" נכון באותה מידה למאזין (הוא שמע זה עתה לאיזה מוקד נרשם),
    // ומחזיר את ההודעה להיות קבועה — כלומר ניתנת להקלטה בקול המערכת.
    key: 'card_center_ended', label: 'החלוקה במוקד שלכם הסתיימה', allowAudio: true,
    defaultText: 'שימו לב, החלוקה במוקד שלכם כבר הסתיימה, ' +
      'אין אפשרות לקבל כעת כרטיס. עמכם הסליחה',
    hint: 'נשמע במקש 2 כשהמוקד של המשפחה סיים לחלק. 🎙️ ניתן ליצור קול טבעי.',
  },
  {
    ...
    //
    // ⚠️ דינמית ולכן allowAudio:false — שם המוקד משתנה בין מאזין למאזין.
    // 🔴 בלי שם המוקד — ולכן ניתנת להקלטה בקול המערכת.
    //
    // ⚠️ כל עוד הנוסח הכיל {center} הוא נקרא כולו ב-TTS של ימות, קול
    // משובש בעברית, לצד הודעות מוקלטות באותה שיחה. "המוקד שבו נרשמתם"
    // מדויק באותה מידה: המאזין הגיע דרך המוקד שלו.
    key: 'card_center_not_open', label: 'המוקד שלכם טרם החל לחלק', allowAudio: true,
    defaultText: 'שימו לב, המוקד שבו נרשמתם טרם החל בחלוקת הכרטיסים. ' +
      'נא לנסות שוב לאחר שתקבלו הודעה על תחילת החלוקה במוקד זה',
    hint: 'נשמע כשהמוקד של המשפחה טרם נפתח לחלוקה. 🎙️ ניתן ליצור קול טבעי.',
  },
