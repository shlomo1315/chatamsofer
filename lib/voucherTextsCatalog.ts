...
export const VOUCHER_TEXT_GROUPS: VoucherTextGroup[] = [
  {
    ...
    fields: [
      ...
      // 🔴 רשימת החנויות המכבדות את הכרטיס משתנה מפעם לפעם. יולדת שקנתה
      // בחנות שכבר אינה ברשימה נתקעת בקופה עם עגלה מלאה — ולכן האזהרה
      // מודגשת ולא נבלעת בהערת התוקף.
      // ⚠️ הטקסט חייב להיכנס בשתי שורות: תחתית השובר צפופה, וטקסט ארוך
      // יותר דורך על הברכה. ראו voucherStoreWarning.test.ts.
      { key: 'card.stores', label: 'אזהרת חנויות (מודגש)', multiline: true,
        default: 'לפני הרכישה ודאו עם המוכרים ובעלי החנות שהם מקבלים את הכרטיס. שינויים ברשימת החנויות נעשים מפעם לפעם, ועל מנת למנוע עגמת נפש ודאו לפני הרכישה שהחנות מקבלת את הכרטיס!' },
