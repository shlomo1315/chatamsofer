...
export function lineageOrderFixEmail(opts: {
  ...
  const body = `
    ...
    <!-- ⚠️ 30 יום — חייב להתאים ל-LINK_DAYS ב-send-lineage-link. הנוסח הקודם
         הבטיח 7 ימים, וזה בדיוק הזמן שבו משפחות מצאו את הקישור סגור. -->
    <p style="margin:0;color:#94a3b8;font-size:12px;">הקישור אישי ותקף 30 יום. לאחר התיקון יועבר לאישור המזכירות.</p>
...
// ─── טופס אישור רב — המייל שנושא את הטופס להחתמה ──────────────────────────────
//
...
export function rabbiFormEmail(opts: {
  ...
  const amountLine = opts.amount != null
    ? `<p style="margin:0 0 20px;color:#334155;font-size:15px;line-height:1.9;">
         ...
         נשמרה במערכת וממתינה לטופס אישור הרב.
       </p>`
    : ''
  ...
  const body = `
    ...
    <p style="margin:0 0 22px;color:#334155;font-size:15px;line-height:1.9;">
      מצורף למייל זה טופס אישור רב. יש להדפיס אותו, להחתים את הרב, ולהחזירו אלינו לפי ההוראות שלהלן.
    </p>
  ...
  return {
    subject: `טופס אישור רב · בקשת הלוואה #${opts.code}`,
    html: shell({
