...
export async function sendMaternityInquiry(
  ...
  const sent = await deliverMail(email, subject, html, undefined, {
    ...
    // 🔴 נרשם ביומן המיילים — היה skipLog:true.
    //
    // ⚠️ בלי הרישום אין שום עקבה: לא ב-sent_emails, לא ב-email_events (שני
    // היומנים נשענים על resend_id שלא נשמר), ולכן לשאלה "האם הבירור הגיע
    // בכלל?" לא הייתה תשובה. תיק שממתין לתשובה יורד מ"ממתין לאישור", כך
    // שמייל שלא הגיע משאיר אותו תקוע בלי שאיש יבחין (15.09).
    //
    // ⚠️ ההודעה נרשמת ממילא ב-maternity_messages לצורך השרשור; הרישום כאן
    // הוא על *המסירה* — נפתח, נדחה, חזר. זה מה שחסר.
