...
export function spokenCode(code: string): string {
  ...
  const DIGIT_GAP = ' ,, '
  // הפסקה ארוכה (~3 שניות) בין ההקראה הראשונה להכרזה על ההקראה השנייה.
  const LONG_GAP = ' ,, ,, ,, ,, ,, ,, '
  const spoken = digits.join(DIGIT_GAP)
  // מבנה: הקראה ראשונה → ~3 שניות → הכרזה → הקראה שנייה (זהה, איטית)
  return [
    'קוד הכניסה שלך הוא',
    DIGIT_GAP,
    spoken,
    LONG_GAP,
    'אני מקריא לכם שוב את הקוד לכניסה למערכת',
    DIGIT_GAP,
    spoken,
  ].join(' ')
}
