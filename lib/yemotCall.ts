...
export function spokenCode(code: string): string {
  ...
  // ⚠️ ללא נקודות: הנקודה היא מפריד ההודעות של id_list_message בימות
  // ("לא להכניס בטקסט את התווים נקודה וקו מפריד" — תיעוד ימות), והיא
  // פיצלה את ההודעה לטוקנים שבורים. ההאטה נעשית בפסיקים (slowText).
