...
function spokenCode(code: string): string {
  // הקראה איטית — כ-שנייה בין ספרה לספרה. בשיחה יוצאת פסיקים/נקודות חותכים את
  // ההודעה, לכן ההשהיה מושגת ברצף רווחים ארוך (מנוע ה-TTS משהה על רווחים). כל
  // ספרה נקראת כמילה מלאה בעברית, כך שההפרדה ברורה מאוד.
  const GAP = '                                             ' // ~45 רווחים ≈ שנייה
