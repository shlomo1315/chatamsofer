...
// ⚠️⚠️ קריטי — ב-webhook (id_list_message) גם פסיק וגם נקודה חותכים/מפצלים את
// ההודעה וגורמים לניתוק/השהיה. אין להשתמש בשום סימן פיסוק! המילים מופרדות
// ברווחים בלבד. ההאטה מושגת בכך שכל ספרה נקראת כמילה עברית מלאה (אפס/אחת/…).
...
export function spokenCode(code: string): string {
  ...
  // רווחים בלבד — בלי פסיקים/נקודות (הם חותכים את ההודעה בימות).
  const words = digits.join(' ')
  return `קוד הכניסה שלך הוא ${words} ואחזור שנית קוד הכניסה שלך הוא ${words}`
}
