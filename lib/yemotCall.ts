...
// ⚠️⚠️ ההקראה ב-webhook דרך id_list_message. הנקודה '.' מפרידה בין הודעות ויוצרת
// הפסקה — זה מנגנון ההאטה שנשמע טוב (ספרה-אחרי-ספרה). פסיק ',' לעומת זאת חותך
// את ההודעה — אסור. הבעיה היחידה: ימות משהה לפני ההודעה הראשונה, ולכן הפתיח
// והספרה הראשונה מחוברים ברווח (הודעה אחת שמתחילה מיד), והנקודות רק *בין הספרות*.
...
export function spokenCode(code: string): string {
  ...
  // נקודה בין הספרות = הפסקה (האטה). הפתיח מחובר לספרה הראשונה ברווח (בלי נקודה
  // לפניה) — כך ההודעה הראשונה מתחילה מיד עם המענה, בלי המתנה.
  const spoken = digits.join('. ')
  return `קוד הכניסה שלך הוא ${spoken}. ואחזור שנית. קוד הכניסה שלך הוא ${spoken}`
}
