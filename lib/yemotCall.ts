...
// ⚠️⚠️ ההקראה נעשית ב-webhook דרך id_list_message. מנגנוני ההפסקה בימות שם:
//   • נקודה '.' — מפריד *בין הודעות נפרדות* ב-id_list_message. גורמת להשהיה
//     ארוכה (~שניות) בין כל "הודעה". אין להשתמש בה בתוך ההקראה!
//   • פסיק ',' בודד מוקף רווחים — הפסקה קצרה בתוך הודעה אחת = הקראה איטית וברורה.
//     זהו המנגנון התקני (ראה slowTokenOf ב-yemot-maternity, פעיל בפרודקשן).
// לכן: פסיק בודד בין המילים, כטוקן/הודעה אחת רציפה (בלי נקודות מפרידות).
...
export function spokenCode(code: string): string {
  ...
  // פסיק בודד בין כל מילה = הפסקה קצרה (הקראה איטית), הכל בהודעה אחת רציפה.
  const parts = ['קוד', 'הכניסה', 'שלך', 'הוא', ...digits, 'ואחזור', 'שנית', ...digits]
  return parts.join(' , ')
}
