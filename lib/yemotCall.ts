...
// ⚠️⚠️ מנגנון ההשהיה בימות:
//   • פסיק ',' — חותך את ההודעה! (הטקסט נקטע והספרות לא מוקראות). אסור בהחלט.
//   • רווחים — מתכווצים ולא יוצרים השהיה. חסרי תועלת.
//   • נקודה '.' — הפסקה מובנית בין המשפטים. נקודה אחת בין ספרה לספרה מאיטה את
//     ההקראה ומפרידה אותה ספרה-ספרה. זה מנגנון ההאטה שאנו משתמשים בו.
// לכן: מסירים פסיקים ותווים מסוכנים, אבל *משאירים נקודות* (הן ההשהיה).
const DIGIT_WORDS = ['אפס', 'אחת', 'שתיים', 'שלוש', 'ארבע', 'חמש', 'שש', 'שבע', 'שמונה', 'תשע']
// מנקה טקסט הקראה תוך שמירה על נקודות (מנגנון ההשהיה). מסיר פסיק/מקף/גרשיים וכו'.
function ttsSafeKeepDots(text: string): string {
  return String(text ?? '').replace(/[,\-"'&|=]/g, ' ').replace(/[ \t]+/g, ' ').trim()
}
export function spokenCode(code: string): string {
  // נקודה בין כל שתי ספרות = הפסקה, כדי שההקראה תהיה איטית וספרה-אחרי-ספרה.
  const words = code.replace(/\D/g, '').split('').map(d => DIGIT_WORDS[Number(d)] ?? d).join('. ')
  // מבנה: הקראה ראשונה → הפסקה → הכרזה → הקראה שנייה (זהה, איטית).
  return ttsSafeKeepDots(`קוד הכניסה שלך הוא. ${words}. ואחזור שנית. קוד הכניסה שלך הוא. ${words}`)
}
