...
//
// ─────────────────────────────────────────────────────────────────────────────
// 🔴 לידה שקטה מקבלת כרטיס מזון בדיוק כמו כל לידה אחרת.
//
// ⚠️ עד 10.09 התור החריג אותה (birth_type='silent'), והנתונים מראים
// שההחרגה מעולם לא שיקפה את המדיניות: מתוך ארבע הלידות השקטות הפעילות,
// שלוש כבר קיבלו 600 ₪ (הרצוג, פרנקל, זלקוביץ). דורמשקין הייתה היחידה
// שנחסמה — ורק משום שהמלאי אזל ברגע האישור, כך שנדרשה ריצה חוזרת של
// התור, ושם ההחרגה תפסה אותה.
//
// ⚠️ הסתירה הייתה גלויה בקוד עצמו: maternityVoucher בונה שובר ייעודי
// ללידה שקטה ("פרטי המבקשת" במקום "פרטי היולדת", בלי ברכות) — כלומר
// המערכת מנפיקה לה שובר כרטיס, ובו בזמן מסרבת לטעון אותו.
//
// ⚠️ ההבדל בלידה שקטה הוא *הניסוח*, לא הזכאות. מה שכן שונה מוסדר במקומו:
// הבתים הפתוחים לה (emailRequestIntake) והשובר עצמו (maternityVoucher).
// ─────────────────────────────────────────────────────────────────────────────
...
export function isAwaitingCard(a: AwaitingAid): boolean {
  return (
    ...
    // 🔴 לידה שקטה *כן* מקבלת כרטיס מזון — ראו ההערה למטה.
