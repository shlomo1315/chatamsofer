...
export const TOOL_DEFS = [
  ...
  },
  {
    ...
    description:
      'סוקר את קו היוחסין של משפחה שנרשמה (בעיקר כזו שממתינה לאישור) מול כלל העץ, ומחזיר נתונים ' +
      'ומגמות: עוגן מאומת בעץ, כמה משפחות רשומות על אותו ענף, כמה על אותה שורה, פילוח השורות, ' +
      'כמה דורות חדשים הוסיפה, וציון התאמה. ⚠️ נתונים לסקירה בלבד — אל תמליץ לאשר או לדחות, רק הצג ' +
      'את הממצאים. השתמש בזה לשאלות כמו "סקור את היוחסין של X", "כמה אמין הנרשם X?". ' +
      'קבל beneficiaryId, או name, או idNumber.',
    input_schema: {
      type: 'object' as const,
      properties: {
...
function buildQuery(
  ...
  if (input.status && spec.statusCol) {
    ...
    // ⚠️ טיוטות שממתינות לטופס אישור רב אינן בקשות שהוגשו: המבקש הוריד
