// ─────────────────────────────────────────────────────────────────────────────
// החזרת טופס אישור רב במייל — זיהוי הבקשה שאליה הטופס שייך.
//
...
export function rabbiFormSubject(loanId: string): string {
  return `טופס אישור רב · בקשת הלוואה #${loanFormCode(loanId)}`
}

/**
 * האם המייל הנכנס נראה כהחזרת טופס אישור רב.
 *
 * ⚠️ מקבל גם את הנוסח הישן ("חתימת רב") ולא רק את הנוכחי ("אישור רב"):
 * הטופס שונה שמו, אבל מיילים שיצאו לפני השינוי עדיין בדרך חזרה — הנושא
 * שלהם נקבע ברגע השליחה. צמצום לנוסח החדש בלבד היה מפיל בשקט כל טופס
 * שנשלח לפני השינוי, וזו תקלה שאיש לא היה מבחין בה עד שמבקש היה מתלונן
 * שהטופס שלו "לא הגיע".
 */
export function looksLikeRabbiFormReturn(subject: string): boolean {
  return /טופס\s*(אישור|חתימת)\s*רב/.test(String(subject ?? ''))
}
