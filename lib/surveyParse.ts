...
const QUOTE_MARKERS: RegExp[] = [
  /^On .+ wrote:/im,
  // Gmail בעברית: "בתאריך יום א׳, 12 ביולי 2026 ב-23:21 מאת היכל החתם סופר <...>"
  // שים לב: Gmail מייצר "מאת", לא "כתב" — הרג'קס חייב לתפוס את שניהם.
  // ⚠️ בלי עוגן ^: Gmail מקדים לשורה תווי ציטוט (>) וסימוני כיווניות (RLM),
  // ואז העוגן לא תואם והציטוט נשאר בגוף ההודעה. גם ההגבלה ל-80 תווים הוסרה —
  // הכותרת בעברית ארוכה יותר ("בתאריך יום ג׳, 14 ביולי 2026 ב-23:49 מאת ...").
  // אחרי "כתב" מגיעות נקודתיים ולא רווח ("…סופר כתב:"), אחרי "מאת" מגיע רווח
  // ("מאת היכל…") — שניהם חייבים להיתפס.
  /בתאריך\s[\s\S]{0,160}?(מאת|כתב)[\s:]/im,
  /^ב.{0,60}?\s(מאת|כתב)[\s:]/im,
  ...
  /^Sent from my /im,
  /^נשלח מ/im,
  // שורה שכולה כתובת מייל בסוגריים משולשים — תחילת ציטוט אצל חלק מהלקוחות
  /^\s*<[^>]+@[^>]+>\s*$/m,
]
