...
// ─────────────────────────────────────────────────────────────────────────────
// 🔴 מחיקת תיק מאושר השאירה כרטיס מנוכה לצמיתות.
//
// אישור לידה מנכה כרטיס; ביטול אישור מחזיר אותו. אבל *מחיקת* התיק לא
// החזירה — ועם התיק נמחק גם הקישור, כך שאין למי להחזיר בדיעבד. זה מקור
// ההפרש "כרטיסים נוכו ואינם מגובים בלידה מאושרת", והדרך היחידה לסגור
// אותו היא ספירה פיזית.
// ─────────────────────────────────────────────────────────────────────────────
describe('🔴 החזרת כרטיס במחיקת תיק', () => {
  /** הסכום נטו של תנועות התיק — מה שנתיב release-card מחשב. */
  const netFor = (rows: { delta: number; aid_id: string | null }[], aidId: string) =>
    rows.filter(r => r.aid_id === aidId).reduce((s, r) => s + r.delta, 0)

  it('🔴 תיק שנוכה עבורו כרטיס — מוחזר בדיוק אחד', () => {
    const rows = [{ delta: -1, aid_id: 'a1' }]
    expect(netFor(rows, 'a1')).toBe(-1)   // ⇒ יוחזר 1
  })

  it('⚠️ תיק שכבר הוחזר — לא מוחזר שוב', () => {
    // 🔴 בלי הבדיקה הזו, ניסיון מחיקה שני היה *מוסיף* כרטיס שלא היה קיים.
    const rows = [{ delta: -1, aid_id: 'a1' }, { delta: 1, aid_id: 'a1' }]
    expect(netFor(rows, 'a1')).toBe(0)    // ⇒ לא יוחזר דבר
  })

  it('⚠️ תיק שלא נוכה עבורו כרטיס — לא מוחזר דבר', () => {
    // תיק שנמחק בעודו "ממתין" לא ניכה כלום; החזרה הייתה יוצרת עודף.
    expect(netFor([], 'a1')).toBe(0)
  })

  it('⚠️ תנועות של תיקים אחרים אינן נספרות', () => {
    const rows = [{ delta: -1, aid_id: 'a1' }, { delta: -1, aid_id: 'a2' }, { delta: -5, aid_id: null }]
    expect(netFor(rows, 'a1')).toBe(-1)
  })

  it('⚠️ תיק עם כמה ניכויים — מוחזר הסכום המלא', () => {
    // תאומים / ניסיון חוזר: אם נוכו שניים, שניהם חוזרים.
    const rows = [{ delta: -1, aid_id: 'a1' }, { delta: -1, aid_id: 'a1' }]
    expect(netFor(rows, 'a1')).toBe(-2)
  })
})
