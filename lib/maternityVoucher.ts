...
async function renderFoodCard(input: VoucherInput): Promise<string> {
  ...
  y = paragraph(c,
    singleCenter
      ? 'יש לגשת אך ורק למוקד הרשום לעיל, בימים ובשעות הנקובים בלבד — אין להגיע במועד אחר.'
      : 'תוכלו לבחור בכל מוקד לקבלת הכרטיס.',
    W - MX, y, W - MX * 2, 10, NAVY, 2)
  y += 2
  ...
  // ── אזהרת החנויות — מודגשת, מעל בלוק ההערות ──
  // 🔴 רשימת החנויות המכבדות את הכרטיס משתנה מפעם לפעם, ויולדת שקנתה
  // בחנות שכבר אינה ברשימה נתקעת בקופה עם עגלה מלאה.
  //
  // ⚠️ מעוגנת לתחתית ולא זורמת מ-y של התוכן שמעליה — בדיוק כמו בלוק
  // ההערות. בשובר עם רשימת מוקדים מלאה התוכן יורד נמוך, וטקסט שזורם
  // היה נדרס. כאן הגובה קבוע, והבלוק שמעליו מצטמצם.
  // ⚠️ אדום ולא נייבי: זו אזהרה מעשית ולא הערה, והיא חייבת להיקרא לפני
  // הכניסה לחנות. הצבע מבדיל אותה מהערת התוקף שמתחתיה.
  // ⚠️ מודגש (boldRightText) ולא paragraph רגיל: זו אזהרה מעשית שנועדה
  // להיקרא לפני הכניסה לחנות, ולא הערה שנסרקת בדיעבד.
  {
    const FS = 9
    const storesText = vtext('card.stores', 'לפני הרכישה ודאו עם המוכרים ובעלי החנות שהם מקבלים את הכרטיס. שינויים ברשימת החנויות נעשים מפעם לפעם, ועל מנת למנוע עגמת נפש ודאו לפני הרכישה שהחנות מקבלת את הכרטיס!')
    const lines = wrapText(storesText, W - MX * 2, s => c.font.widthOfTextAtSize(toVisual(s), FS))
    let sy = 92
    for (const ln of lines) { boldRightText(c, ln, W - MX, sy, FS, RED); sy -= FS + 2 }
  }
