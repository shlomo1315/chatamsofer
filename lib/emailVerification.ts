...
export function isValidEmail(email: string | null | undefined): boolean {
  ...
  // 🔴 טלפון שנדחס לתוך שדה המייל.
  //
  // הדפוס שהתגלה ביבוא לרשימת הדיוור: הכתובת והטלפון הודבקו יחד לשדה אחד
  // ("0527166134@gmail.com30005000", "0583297761,r@gmail.com"). הרגקס הישן
  // קיבל את כולן — "com30005000" הוא אכן רצף שאינו רווח ואינו @ — ורק
  // מערכת הדיוור החיצונית דחתה אותן, אחרי שכבר נשמרו אצלנו.
  //
  // ⚠️ פסיק אינו חוקי בכתובת מייל רגילה, והוא הסימן המובהק לשתי כתובות
  // (או כתובת+טלפון) שנדחסו יחד.
  if (/[,;]/.test(e)) return false

  // שם המשתמש חייב להתחיל ולהסתיים בתו אלפאנומרי: ".@gmail.com" עבר קודם.
  if (!/^[A-Za-z0-9](.*[A-Za-z0-9])?$/.test(local)) return false

  // ⚠️ הדומיין נבדק לפי מבנה אמיתי: תוויות אלפאנומריות מופרדות בנקודות,
  // וסיומת של 2-24 אותיות בלבד. זה מה שפוסל "gmail.com30005000" ו-"g.k":
  // ספרות בסיומת אינן קיימות בשום TLD, וסיומת בת תו אחד אינה קיימת.
  if (!/^[A-Za-z0-9]([A-Za-z0-9-]*[A-Za-z0-9])?(\.[A-Za-z0-9]([A-Za-z0-9-]*[A-Za-z0-9])?)*\.[A-Za-z]{2,24}$/.test(domain)) {
    return false
  }
  return true
}
