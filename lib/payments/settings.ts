...
export interface PaymentSettings {
  ...
  testMode?: boolean
  /**
   * קטגוריה קבועה בנדרים (שדה Groupe בדף התשלום) — כדי שכל תשלומי היריד
   * ייכנסו תחת קטגוריה אחת בממשק הניהול של המוסד, נפרדת משאר התרומות.
   *
   * ⚠️ טקסט חופשי מבחינת נדרים — אינו חייב להתאים לרשימת הקטגוריות
   * שהוגדרה שם. הנעילה (GroupeLock) מוצמדת תמיד כשהשדה מוגדר, כדי
