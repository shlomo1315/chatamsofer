...
| גיבוי יומי ל-Google Drive | `https://<דומיין>/api/cron/backup?secret=<CRON_SECRET>` | כל לילה ב-00:00 (`0 0 * * *`) |

> **גיבוי:** הגיבוי רץ כל לילה. הניקוי של הגיבויים הישנים מתבצע אוטומטית
> **פעם בחודש** (בריצה הראשונה של חודש חדש) — נמחקים כל הגיבויים מהחודשים
> הקודמים ונשמר רק החודש הנוכחי (תמיד לפחות 7 האחרונים, כרשת ביטחון).
> דורש: `GOOGLE_DRIVE_BACKUP_FOLDER_ID` + חיבור Google פעיל.
